...
template<class T>
using matrix = matrix_fixed_base<T, 4, 4>;
...
int main(){
	...
	using U = M;
	auto UU = multiplies<>();
	U U_id = M::multiplicative_identity(2, 2);
	auto Uinv = [&](const U &f)->U{
		return *f.inverse();
	};
	auto UT = [&](U f, const T &x)->T{
		return {f * get<0>(x) / f, f * get<1>(x) / f, get<2>(x)};
	};
	auto lct = make_PQPU_LCT<false>(TT, T_id, Tflip, UU, U_id, UT);
	auto read = [&]()->M{
		M mat(2, 2);
		for(auto i = 0; i < 2; ++ i){
			for(auto j = 0; j < 2; ++ j){
